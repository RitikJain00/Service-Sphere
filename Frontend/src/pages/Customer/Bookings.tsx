...
const CustomerBookings = () => {
  ...
  pastOrders.sort((a, b) => {
    const parseDate = (dateStr: string) => {
      const [day, month, year] = dateStr.split('/').map(Number);
      const fullYear = year < 50 ? 2000 + year : 1900 + year; // Handle YY format properly
      return new Date(fullYear, month - 1, day); // Month is 0-based in JS Date
    };
    ...
    <div className="flex flex-col min-h-[100vh] bg-mine-shaft-950">
      ...
      <div className="text-4xl text-bright-sun-400 font-bold text-center">Upcomming Bookings</div>
        ...
        {upcommingOrders.length > 0 ? (
          ...
          <Carousel slideSize="22%" align='center' slideGap="md"  dragFree controlSize={30}  loop 
          ...
          { upcommingOrders.map((job) => <Carousel.Slide key={job.service.id}>
      ...
      <div className="px-8 flex-grow mt-4">
        {pastOrders.length > 0 ? (
...
<Table.Thead>
  ...
  <Table.Tbody> 
    { pastOrders.map((job,index) =>  
      <Table.Tr key={index} className="text-lg md:text-xl font-bold text-center">
      <Table.Td className="flex items-center justify-center"> <img className="w-24 h-24 " src={job.service.image} alt="" /> </Table.Td>
